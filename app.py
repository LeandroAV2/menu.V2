...
import hashlib, datetime, os, sqlite3, json
...
def init_db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS usuarios (
        ...
        password TEXT NOT NULL, creado_en TEXT NOT NULL,
        puntos INTEGER NOT NULL DEFAULT 0)""")
    try: c.execute("ALTER TABLE usuarios ADD COLUMN puntos INTEGER NOT NULL DEFAULT 0")
    except: pass
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS pedidos (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        usuario_email TEXT NOT NULL, usuario_nombre TEXT NOT NULL,
        ...
        hora TEXT NOT NULL)""")
    for col in [("puntos_ganados","INTEGER NOT NULL DEFAULT 0"),
                ("tipo","TEXT NOT NULL DEFAULT 'local'"),
                ("pago","TEXT NOT NULL DEFAULT 'efectivo'"),
                ("estado","TEXT NOT NULL DEFAULT 'pendiente'")]:
        try: c.execute(f"ALTER TABLE pedidos ADD COLUMN {col[0]} {col[1]}")
        except: pass

    c.execute("""CREATE TABLE IF NOT EXISTS canjes (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        usuario_email TEXT NOT NULL, beneficio_id INTEGER NOT NULL,
        beneficio_nombre TEXT NOT NULL, puntos_usados INTEGER NOT NULL, hora TEXT NOT NULL)""")

    # Admin por defecto
    try:
        c.execute("INSERT INTO admins (nombre,email,password,creado_en) VALUES (?,?,?,?)",
...
def get_puntos(email):
    conn = get_db()
    row = conn.execute("SELECT puntos FROM usuarios WHERE email=?", (email,)).fetchone()
    conn.close()
...
def registro():
    ...
    try:
        conn = get_db()
        conn.execute("INSERT INTO usuarios (nombre,email,password,creado_en,puntos) VALUES (?,?,?,?,?)",
...
def pedido():
    ...
    tipo  = datos.get("tipo", "local")     # "local" | "llevar"
    pago  = datos.get("pago", "efectivo")  # "efectivo" | "transferencia"
    ...
    conn = get_db()
    conn.execute(
        "INSERT INTO pedidos (usuario_email,usuario_nombre,items,total,puntos_ganados,tipo,pago,estado,hora) VALUES (?,?,?,?,?,?,?,?,?)",
        (usuario["email"], usuario["nombre"], json.dumps(items), total, puntos_ganados,
         tipo, pago, "pendiente", datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S")))
    conn.execute("UPDATE usuarios SET puntos = puntos + ? WHERE email = ?", (puntos_ganados, usuario["email"]))
    conn.commit()
    nuevos_puntos = conn.execute("SELECT puntos FROM usuarios WHERE email=?", (usuario["email"],)).fetchone()["puntos"]
    conn.close()
...
def canjear():
    ...
    conn = get_db()
    puntos_actuales = conn.execute("SELECT puntos FROM usuarios WHERE email=?", (usuario["email"],)).fetchone()["puntos"]
    if puntos_actuales < beneficio["puntos"]:
        conn.close(); return jsonify({"error":"Puntos insuficientes"}), 400
    conn.execute("UPDATE usuarios SET puntos = puntos - ? WHERE email = ?", (beneficio["puntos"], usuario["email"]))
    conn.execute("INSERT INTO canjes (usuario_email,beneficio_id,beneficio_nombre,puntos_usados,hora) VALUES (?,?,?,?,?)",
...
def admin_login():
    ...
    if request.method == "POST":
        ...
        password = request.form.get("password","")
        conn = get_db()
        a = conn.execute("SELECT * FROM admins WHERE email=?", (email,)).fetchone()
        conn.close()
...
def admin_api_pedidos():
    ...
    conn = get_db()
    if estado == "todos":
        rows = conn.execute("SELECT * FROM pedidos ORDER BY id DESC LIMIT 100").fetchall()
    else:
        rows = conn.execute("SELECT * FROM pedidos WHERE estado=? ORDER BY id DESC LIMIT 100", (estado,)).fetchall()
    conn.close()
    pedidos = []
    for r in rows:
        pedidos.append({
...
def admin_cambiar_estado(pid):
    ...
    conn = get_db()
    conn.execute("UPDATE pedidos SET estado=? WHERE id=?", (nuevo_estado, pid))
    conn.commit()
    conn.close()
...
def admin_stats():
    ...
    total_hoy = conn.execute(
        "SELECT COALESCE(SUM(total),0) as t FROM pedidos WHERE hora LIKE ?", (hoy+"%",)).fetchone()["t"]
    cant_hoy = conn.execute(
        "SELECT COUNT(*) as c FROM pedidos WHERE hora LIKE ?", (hoy+"%",)).fetchone()["c"]
    pendientes = conn.execute(
        "SELECT COUNT(*) as c FROM pedidos WHERE estado='pendiente'").fetchone()["c"]
    en_prep = conn.execute(
        "SELECT COUNT(*) as c FROM pedidos WHERE estado='en_preparacion'").fetchone()["c"]
    conn.close()
